...
var (
	...
	cacheImUsercenterUserIdPrefix     = "cache:imUsercenter:user:id:"
	cacheImUsercenterUserMobilePrefix = "cache:imUsercenter:user:mobile:"
)
...
func (m *defaultUserModel) Insert(ctx context.Context, session sqlx.Session, data *User) (sql.Result, error) {
	...
	imUsercenterUserIdKey := fmt.Sprintf("%s%v", cacheImUsercenterUserIdPrefix, data.Id)
	imUsercenterUserMobileKey := fmt.Sprintf("%s%v", cacheImUsercenterUserMobilePrefix, data.Mobile)
	return m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?, ?, ?, ?, ?, ?, ?)", m.table, userRowsExpectAutoSet)
		if session != nil {
			return session.ExecCtx(ctx, query, data.DeleteTime, data.DelState, data.Version, data.Mobile, data.Password, data.Nickname, data.Sign, data.Avatar, data.Info)
		}
		return conn.ExecCtx(ctx, query, data.DeleteTime, data.DelState, data.Version, data.Mobile, data.Password, data.Nickname, data.Sign, data.Avatar, data.Info)
	}, imUsercenterUserIdKey, imUsercenterUserMobileKey)
}

func (m *defaultUserModel) FindOne(ctx context.Context, id int64) (*User, error) {
	imUsercenterUserIdKey := fmt.Sprintf("%s%v", cacheImUsercenterUserIdPrefix, id)
	var resp User
	err := m.QueryRowCtx(ctx, &resp, imUsercenterUserIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultUserModel) FindOneByMobile(ctx context.Context, mobile string) (*User, error) {
	imUsercenterUserMobileKey := fmt.Sprintf("%s%v", cacheImUsercenterUserMobilePrefix, mobile)
	var resp User
	err := m.QueryRowIndexCtx(ctx, &resp, imUsercenterUserMobileKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
...
func (m *defaultUserModel) Update(ctx context.Context, session sqlx.Session, newData *User) (sql.Result, error) {
	...
	return m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.Mobile, newData.Password, newData.Nickname, newData.Sign, newData.Avatar, newData.Info, newData.Id)
		}
		return conn.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.Mobile, newData.Password, newData.Nickname, newData.Sign, newData.Avatar, newData.Info, newData.Id)
	}, imUsercenterUserIdKey, imUsercenterUserMobileKey)
}
...
func (m *defaultUserModel) UpdateWithVersion(ctx context.Context, session sqlx.Session, newData *User) error {
	...
	sqlResult, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.Mobile, newData.Password, newData.Nickname, newData.Sign, newData.Avatar, newData.Info, newData.Id, oldVersion)
		}
		return conn.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.Mobile, newData.Password, newData.Nickname, newData.Sign, newData.Avatar, newData.Info, newData.Id, oldVersion)
	}, imUsercenterUserIdKey, imUsercenterUserMobileKey)
	if err != nil {
		return err
	}
...
func (m *defaultUserModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheImUsercenterUserIdPrefix, primary)
}
